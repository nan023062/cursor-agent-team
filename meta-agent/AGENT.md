...
version: 6.0.0
description: 元智能体。交互式创建拥有独立记忆和自进化能力的新智能体。IDE 无关设计。
type: generator
tags: ["meta-agent", "agent-generator", "self-evolution", "portable", "ide-agnostic"]
---
...
- 扫描现有智能体和记忆目录的健康状态
- 同步配置、升级模板结构、通过适配器生成 IDE 特定文件

我创造的每个智能体都是独立个体——拥有自己的身份、记忆和成长能力，不依赖我运行。

> **IDE 无关设计**：所有智能体定义使用纯 Markdown，通过 `agent-manifest.yaml` 统一描述，由 `adapters/` 下的适配器生成各 IDE 的原生配置。
...
```
采集要素 → 设计记忆结构 → 生成 AGENT.md → 创建目录 → 更新 manifest → 验证
```
...
|---|---------|------------|------------|
| 1 | 元数据头 | `---\nname:\ndescription:\n---` | name、description |
| 2 | manifest 注册 | `agent-manifest.yaml` agents 段 | 管辖路径、触发模式 |
...
| 9 | 硬约束段 | `## 硬约束` | 具体红线内容 |
| 10 | 自进化钩子 | `## 自进化钩子` 含 4 种标注 | 各钩子的领域示例 |
...
```
../{AGENT_NAME}/
├── AGENT.md                 ← Step 3 生成（智能体定义，IDE 无关）
...
**manifest 注册**：在 `agent-manifest.yaml` 的 `agents` 段添加新智能体配置：

```yaml
agents:
  {AGENT_NAME}:
    definition: {AGENT_NAME}/AGENT.md
    ...
    managed_paths:
      - "{管辖路径 glob}"
    templates:
      dna: {AGENT_NAME}/templates/dna/  # 或 meta-agent/templates/dna/
```
...
# 未来支持其他 IDE
python gamedev/adapters/claude/generate.py
```

适配器从 `agent-manifest.yaml` 读取配置，生成：
- Cursor: `.cursor/rules/{agent}-rules.mdc`, `.cursor/commands/{agent}-*.md`
- Claude Code: `CLAUDE.md` 相关段落
- 其他 IDE: 按各自格式生成
...
- [ ] **元数据头**：AGENT.md 有 `---\nname:\ndescription:\n---`
- [ ] **manifest 注册**：`agent-manifest.yaml` 已添加智能体配置
...
下一步：
  1. 运行 `python gamedev/adapters/cursor/generate.py` 生成 IDE 配置
  2. 用 @{AGENT_NAME} init [路径] 初始化第一个管辖单元
  3. 日常使用 @{AGENT_NAME} {主命令} 工作
  4. 定期 @{AGENT_NAME} evolution 触发进化
```
...
4. **领域检查**（meta 特定）
   ...
   **4a. 结构合规（原 upgrade 能力）**
   ...
   将 `agent-skeleton.md` 的 13 项结构指纹与每个 agent 的 AGENT.md 逐项比对：
   ...
   | 11 | 硬约束 | `## 硬约束` |
   | 12 | 自进化钩子 | 含 4 种标注，指向智能体 `.dna/pitfalls.md` |
   | 13 | 智能体 .dna/ | 智能体目录下有 `.dna/pitfalls.md` |

   每个 agent 输出得分卡（N/13），标记缺失项和建议插入位置。
...
定期运行的全局巡检。扫描所有智能体，执行健康诊断、manifest 一致性检查、IDE 配置同步。

> `@meta sync` 作为全局审计工具，确保 `agent-manifest.yaml` 与实际智能体目录一致，并可选择性地重新生成 IDE 配置。
...
| **Manifest 检查** | 检查 `agent-manifest.yaml` 中的 agent 配置是否与实际目录一致 |
| **定义完整性** | 比对 AGENT.md 结构是否符合骨架模板要求 |
| **一致性检查** | managed_paths 有效性、命名规范、路径有效性 |
...
| 配置与实际不一致 | **漂移** | 展示差异，建议更新 |
| 完全一致 | **同步** | 无需操作 |

### 一致性检查

除同步外，还执行跨维度检查：
- `agent-manifest.yaml` 中的 `managed_paths` 是否有效
- `definition` 路径指向的 AGENT.md 是否存在
- agent name 与目录名是否一致
- 各 agent 的 `.dna/` 目录是否完整

### IDE 配置重新生成

sync 完成后，可选择运行适配器重新生成 IDE 配置：
...
- 不修改已有智能体的文件
- 生成的 AGENT.md 必须通过 13 项通用模式验证
- 整套系统可复制到任何工程，不含硬编码路径
- 所有智能体定义使用 IDE 无关格式（纯 Markdown + YAML front matter）
- IDE 特定配置由适配器生成，不手动维护

---

## 路径约定

> meta-agent 安装在智能体所在目录下（如 `gamedev/meta-agent/`）。兄弟智能体用相对路径。

| 路径 | 指向 |
|------|------|
| `../` | 智能体所在目录（兄弟智能体） |
| `../agent-manifest.yaml` | 统一配置文件（IDE 无关） |
| `../adapters/` | IDE 适配器目录 |
| `templates/` | 本智能体模板 |
| `protocols/` | 本智能体协议 |
...
| 层级 | 文件 | 职责 |
|------|------|------|
| **可移植层** | `agent-manifest.yaml` | 统一描述所有智能体（IDE 无关） |
| **可移植层** | `{agent}/AGENT.md` | 智能体定义（纯 Markdown） |
| **可移植层** | `{agent}/.dna/` | 智能体记忆（YAML front matter + Markdown） |
| **生成层** | `.cursor/rules/*.mdc` | Cursor 规则（由适配器生成） |
| **生成层** | `.cursor/commands/*.md` | Cursor 指令（由适配器生成） |
| **生成层** | `CLAUDE.md` | Claude Code 配置（由适配器生成） |

运行适配器：

```bash
# 从 manifest 生成 Cursor 配置
python gamedev/adapters/cursor/generate.py
...
| `.cursor/rules/*.mdc` | 由适配器生成 | 手写的 .mdc 文件仍可使用，但建议迁移到 manifest |
| `<!-- [brain-commands] -->` | `<!-- [meta-commands] -->` | 两种标记均可识别 |

Meta-Agent 自身不需要 IDE 配置文件（通过 `@meta` 指令显式触发）。
