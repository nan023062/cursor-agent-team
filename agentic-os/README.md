...
| **工作区执行器** | 结构化读写 `.dna/` 记忆文件，C# 模块可用 Roslyn 提取 Public API |
...
```
agentic-os/agentic-os/bin/Release/net8.0/agentic-os.exe   ← Windows
agentic-os/agentic-os/bin/Release/net8.0/agentic-os       ← Linux / macOS
```
...
启动 Avalonia 桌面应用，实时查看调用栈与 DNA 拓扑图。需事先设置环境变量 `AGENTIC_OS_PROJECT_ROOT`（或 `DNA_MCP_PROJECT_ROOT`）。
...
检查两个模块之间的依赖是否在 `dependencies.md` 中声明，并验证边界访问权限。

```powershell
agentic-os cli validate Art/Characters Core
# ✓ 依赖关系合法：Art/Characters → Core 已在 dependencies.md 中声明

agentic-os cli validate Design/Combat Art/Characters
# ✗ 未声明依赖：Design/Combat 的 dependencies.md 中不包含 Art/Characters
```
...
## 职责声明
（根据模块类型填写：代码模块用 ## Public API，策划模块用 ## 交付契约，美术模块用 ## 资产规范）
```
...
| 工具 | 说明 |
|------|------|
| `extract_public_api` | 【C# 模块专用】用 Roslyn 精确提取 Public API 签名 |
...
| **SharedOrSoft** | Shared 层或 Soft 边界邻居 | `.dna/` 全部 + 职责声明相关文件路径 |
| **HardDependency** | Hard 边界依赖 | 仅 `architecture.md` 的职责声明段 |
| **Unlinked** | 无依赖关系的模块 | 拦截，返回阻断消息 |

> **职责声明段**因模块类型而异：代码模块为 `## Public API`，策划模块为 `## 交付契约`，美术模块为 `## 资产规范`，均被 OS 自动识别。
...
{模块}/
├── .dna/
│   ├── architecture.md      # 永久规则：定位、职责声明、边界、设计决策
│   ├── pitfalls.md          # 经验记录（结构化标签）
│   ├── changelog.md         # 变更记录
│   ├── dependencies.md      # 依赖白名单
│   └── wip.md               # 进行中任务（跨会话续接）
...
| 文件 | 层级 | 写入频率 | 通用含义 |
|------|------|---------|---------|
| `architecture.md` | L3 永久规则 | 低（仅升格时） | 模块是什么、对外暴露什么、约束是什么 |
| `pitfalls.md` | L1 原始经验 | 高（每次踩坑后） | 在这个模块里踩过的坑 |
| `changelog.md` | 变更记录 | 中（每次迭代后） | 变更历史 |
| `dependencies.md` | 依赖声明 | 低（依赖变更时） | 依赖哪些其他模块 |
| `wip.md` | 任务续接 | 中（跨会话任务） | 当前进行中的工作 |
...
# 临时
$env:PATH += ";$PWD\agentic-os\agentic-os\bin\Release\net8.0"
...
```bash
dotnet build agentic-os/agentic-os.sln -c Release
export PATH="$PATH:$(pwd)/agentic-os/agentic-os/bin/Release/net8.0"
```
