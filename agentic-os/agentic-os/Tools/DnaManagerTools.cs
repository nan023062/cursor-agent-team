...
public class DnaManagerTools(DnaManagerService dnaManager, ProjectConfig config)
{
    ...
    [McpServerTool, Description(
        ...
        "hard 边界只返回职责声明段（Public API / 交付契约 / 资产规范），彻底阻断内部内容访问。" +
        ...
        [Description("项目根目录的绝对路径（留空则使用环境变量 AGENTIC_OS_PROJECT_ROOT）")] string? projectRoot = null)
    {
        ...
        {
            sb.AppendLine("> [物理过滤] 硬边界模块：仅返回职责声明段，内部内容已屏蔽。");
            sb.AppendLine();
        }
    ...
    [McpServerTool, Description(
        "校验模块 A 对模块 B 的依赖是否在 dependencies.md 白名单内，以及指定的接口/职责是否在对方的职责声明段中。" +
        "越界直接返回标准阻断消息。" +
