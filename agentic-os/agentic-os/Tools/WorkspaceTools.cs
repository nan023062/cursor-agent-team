...
public class WorkspaceTools(WorkspaceService workspace, ProjectConfig config)
{
        ...
        [Description("true 则自动写入 architecture.md 的 ## Public API 段，false 则只返回提取结果")] bool writeToArchitecture = false)
    {
        ...
        {
            var writeResult = await workspace.WritePublicApiToArchitectureAsync(modulePath, apiContent);
            return $"{writeResult}\n\n提取内容预览：\n{apiContent}";
        }
    ...
    [McpServerTool, Description(
        "结构化写入 pitfall 教训条目到模块的 pitfalls.md，并自动同步更新全局 pitfall-index.md。确保格式规范、标签一致。" +
        "参数 modulePath：模块目录的绝对路径。" +
    ...
    [McpServerTool, Description(
        "追加变更记录到模块的 changelog.md，自动处理 [BREAKING] 和 [跨模块] 标注。" +
        "参数 modulePath：模块目录的绝对路径。" +
    ...
    [McpServerTool, Description(
        "一次性读取模块的完整 .dna/ 上下文，按加载协议顺序（architecture → pitfalls → dependencies → wip）组装返回。" +
        "参数 modulePath：模块目录的绝对路径。")]
