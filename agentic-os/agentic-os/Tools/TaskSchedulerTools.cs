...
public class TaskSchedulerTools(TaskSchedulerService scheduler, ProjectConfig config)
{
    ...
    [McpServerTool, Description(
        "将当前程序集的开发任务挂起并压入调用栈（Push），保存完整上下文状态，供后续恢复。" +
        "典型场景：开发程序集 A 时发现需要程序集 B 提供新接口，挂起 A，转去开发 B。" +
        "参数 currentAssembly：当前正在开发的程序集名称。" +
        "参数 taskDescription：任务需求描述。" +
        "参数 suspendReason：挂起原因（例如：等待 Core 提供 IEventBus 接口）。" +
        "参数 contextSummary：当前进展摘要（已完成的步骤、关键设计决策、下次恢复时需要知道的信息）。" +
        "参数 resumeCondition：恢复条件（例如：Core/.dna/architecture.md ## Public API 包含 IEventBus）。" +
    ...
    public string suspend_and_push(
        [Description("当前正在开发的程序集名称")] string currentAssembly,
        [Description("任务需求描述")] string taskDescription,
        [Description("挂起原因，例如：等待 Core 提供 IEventBus 接口")] string suspendReason,
        [Description("当前进展摘要：已完成的步骤、关键设计决策、下次恢复时需要知道的信息")] string contextSummary,
        [Description("恢复条件，例如：Core/.dna/architecture.md ## Public API 包含 IEventBus")] string resumeCondition,
        ...
        [Description("项目根目录的绝对路径（留空则使用环境变量 AGENTIC_OS_PROJECT_ROOT）")] string? projectRoot = null)
    {
        ...
        return scheduler.SuspendAndPush(
            config.Resolve(projectRoot), currentAssembly, taskDescription,
            suspendReason, contextSummary, resumeCondition, subTasks);
    }

    [McpServerTool, Description(
        "标记当前程序集任务完成并弹出调用栈（Pop），自动恢复上一层挂起任务的完整上下文。" +
        "参数 completedAssembly：刚完成开发的程序集名称（必须与栈顶一致）。" +
        "参数 projectRoot：项目根目录的绝对路径（留空则使用环境变量 AGENTIC_OS_PROJECT_ROOT）。")]
    public string complete_and_pop(
        [Description("刚完成开发的程序集名称（必须与当前栈顶程序集一致）")] string completedAssembly,
        [Description("项目根目录的绝对路径（留空则使用环境变量 AGENTIC_OS_PROJECT_ROOT）")] string? projectRoot = null)
    {
        var (message, _) = scheduler.CompleteAndPop(config.Resolve(projectRoot), completedAssembly);
        return message;
    }
